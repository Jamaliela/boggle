//Modified from https://codereview.stackexchange.com/questions/146877/boggle-board-solver-in-c 
...
// Map word lengths to Boggle game points
unordered_map<int, int> POINTS{ {3, 1}, {5, 2}, {6, 3}, {7, 5}, {8, 11} };
...
public:
	Dictionary();
	Dictionary(string, unsigned int word_length = 3);
	~Dictionary();
...
Dictionary::Dictionary(string path, unsigned int word_length) {
	string line;
	ifstream file(path);
	if (file.is_open()) {
...
public:
	Boggle(shared_ptr<const Dictionary> dict, int size = 4, unordered_map<int, int> points = POINTS);
	~Boggle();

	void BLoad(string letters);
	void BPrint();
	int BScore();
};

// Boggle board constructor
Boggle::Boggle(shared_ptr<const Dictionary> dict, int size, unordered_map<int, int> points) :
...
void Boggle::BLoad(string letters) {
	int i = 0;
	for (square &it : board) {
		it.value = letters[i];
		i += 1;
...
int main() {
	...
	cout << "working... please be patient..." << endl;

	auto dict = make_shared<const Dictionary>("mydict.txt");
	Boggle b(dict);
	b.BLoad("serspatglinesers");
	b.BPrint();
	cout << "Your score is: " << b.BScore() << endl;
